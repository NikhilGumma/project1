...
# your wrangling code
knitr::kable(moviejoin %>% summarize(Mean = mean(Runtime, 
    na.rm = TRUE), Standard_Deviation = sd(Runtime, na.rm = TRUE), 
    Variance = var(Runtime, na.rm = TRUE), `25%_Quantile` = quantile(Runtime, 
        0.25, na.rm = TRUE), `50%_Quantile` = quantile(Runtime, 
        0.5, na.rm = TRUE), `75%_Quantile` = quantile(Runtime, 0.75, 
        na.rm = TRUE), Minimum = min(Runtime, na.rm = TRUE), Maximum = max(Runtime, 
        na.rm = TRUE), Num_Distinct = n_distinct(Runtime, na.rm = TRUE)) %>% 
    na.omit, caption = "Runtime Statistics of Movies in Joined database")
#IMDb statistics
knitr::kable(moviejoin %>% summarize(Mean = mean(IMDb.Rating.x, 
    na.rm = TRUE), Standard_Deviation = sd(IMDb.Rating.x, na.rm = TRUE), 
    Variance = var(IMDb.Rating.x, na.rm = TRUE), `25%_Quantile` = quantile(IMDb.Rating.x, 
        0.25, na.rm = TRUE), `50%_Quantile` = quantile(IMDb.Rating.x, 
        0.5, na.rm = TRUE), `75%_Quantile` = quantile(IMDb.Rating.x, 0.75, 
        na.rm = TRUE), Minimum = min(IMDb.Rating.x, na.rm = TRUE), Maximum = max(IMDb.Rating.x, 
        na.rm = TRUE), Num_Distinct = n_distinct(IMDb.Rating.x, na.rm = TRUE)) %>% 
    na.omit, caption = "IMDb rating Statistics of Movies in Joined database")
# your wrangling code
knitr::kable(moviejoin %>% group_by(IMDb.Rating.x) %>% pivot_wider() %>% 
    summarize(Mean = mean(IMDb.Rating.x, na.rm = TRUE), Standard_Deviation = sd(IMDb.Rating.x, 
        na.rm = TRUE), Variance = var(IMDb.Rating.x, na.rm = TRUE), `25%_Quantile` = quantile(IMDb.Rating.x, 
        0.25, na.rm = TRUE), `50%_Quantile` = quantile(IMDb.Rating.x, 0.5, 
        na.rm = TRUE), `75%_Quantile` = quantile(IMDb.Rating.x, 0.75, na.rm = TRUE), 
        Minimum = min(IMDb.Rating.x, na.rm = TRUE), Maximum = max(IMDb.Rating.x, na.rm = TRUE), 
        Num_Distinct = n_distinct(IMDb.Rating.x, na.rm = TRUE)) %>% na.omit %>% 
    arrange(desc(Mean)) %>% slice(1:5) %>% select(IMDb.Rating.x), caption = "Top 5 IMDb Ratings")
...
#Mutate Function
HighlyRated_Metric_Added <- moviejoin%>% mutate( HighlyRated = (IMDb.Rating.y>7.5))
...
ggplot(moviejoin, aes(x = IMDb.Rating.x, color = Num..Votes)) + geom_density(alpha = 0.75) + 
...
ggplot(moviejoin, aes(x = IMDb.Rating.y, y = Language, color = Title.type )) + 
...
```{R}
# your plot 3
ggplot(moviejoin, aes(x = Title.type, y = IMDb.Rating.x)) + geom_bar(stat = "summary", 
